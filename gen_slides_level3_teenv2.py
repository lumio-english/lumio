...
from crew_talk_level3 import CREW_TALK
...
for num in sorted(_lessons.keys()):
    ...
    # Describing Time: a picture-description slide at the same 4
    # checkpoint lessons every level uses (6, 11, 16, 20), placed right
    # after the Hook per Eslam's instruction. Only these 4 lessons per
    # level have an image, so every other lesson silently gets nothing
    # (describing_time_image stays None, and build_deck_v2 already
    # skips the slide entirely when it's not provided).
